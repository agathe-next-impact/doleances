...
export async function GET() {
  const [res, articles, pages] = await Promise.all([
    fetch(`${API_BASE_URL}/groupe_local?per_page=100&_fields=slug,date,modified`, {
      next: { revalidate: 3600 },
    }),
    fetch(`${API_BASE_URL}/posts?per_page=100&_fields=slug,status,date,modified`, {
      next: { revalidate: 3600 },
    }),
    fetch(`${API_BASE_URL}/pages?per_page=100&_fields=slug,status,date,modified`, {
      next: { revalidate: 3600 },
    }),
  ]);
