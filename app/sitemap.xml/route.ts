...
export async function GET() {
  const res = await fetch(`${API_BASE_URL}/groupe_local?_embed&per_page=100`, {
    next: { revalidate: 0 }, // ou cache: 'no-store'
  });
  const articles = await fetch(`${API_BASE_URL}/posts?_embed&per_page=100`, {
    next: { revalidate: 0 }, // ou cache: 'no-store'
  });
  const pages = await fetch(`${API_BASE_URL}/pages?_embed&per_page=100`, {
    next: { revalidate: 0 }, // ou cache: 'no-store'
  });
